...
import { getStudents, getRecords, type Student, type AttendanceRecord } from "@/lib/supabase-store";
import {
...
export default function Reports() {
  ...
  useEffect(() => {
    async function load() {
      const [s, r] = await Promise.all([getStudents(), getRecords()]);
  ...
  const stats = useMemo(() => {
    return filteredStudents.map((student) => {
      const sr = filteredRecords.filter((r) => r.studentId === student.id);
      const hadir = sr.filter((r) => r.status === "hadir").length;
      const izin = sr.filter((r) => r.status === "izin").length;
      const sakit = sr.filter((r) => r.status === "sakit").length;
      const alpha = sr.filter((r) => r.status === "alpha").length;
  ...
  const barData = useMemo(() => stats.map(({ student, hadir, izin, sakit, alpha }) => ({
    name: student.name.split(" ")[0], Hadir: hadir, Izin: izin, Sakit: sakit, Alpha: alpha,
  })), [stats]);

  const pieData = useMemo(() => {
    const t = stats.reduce((a, s) => ({ hadir: a.hadir + s.hadir, izin: a.izin + s.izin, sakit: a.sakit + s.sakit, alpha: a.alpha + s.alpha }), { hadir: 0, izin: 0, sakit: 0, alpha: 0 });
    return [{ name: "Hadir", value: t.hadir }, { name: "Izin", value: t.izin }, { name: "Sakit", value: t.sakit }, { name: "Alpha", value: t.alpha }].filter((d) => d.value > 0);
  }, [stats]);
  ...
  const monthlyData = useMemo(() => {
    ...
    const relevant = filteredRecords.filter((r) => studentIds.has(r.studentId));
    ...
    relevant.forEach((r) => {
      ...
      if (!monthMap.has(key)) monthMap.set(key, { hadir: 0, izin: 0, sakit: 0, alpha: 0 });
      monthMap.get(key)![r.status]++;
    });
    return Array.from(monthMap.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([key, val]) => {
      ...
      return { label: `${MONTH_NAMES[parseInt(month)]} ${year}`, ...val, total, percentage: total > 0 ? Math.round((val.hadir / total) * 100) : 0 };
    });
  }, [filteredStudents, filteredRecords]);

  const exportCSV = () => {
    const header = "Nama,Kelas,Hadir,Izin,Sakit,Alpha,Persentase";
    const rows = stats.map(({ student, hadir, izin, sakit, alpha, percentage }) => `"${student.name}","${student.class}",${hadir},${izin},${sakit},${alpha},${percentage}%`);
    const csv = [header, ...rows].join("\n");
    const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `laporan-absensi${selectedClass !== "all" ? `-${selectedClass}` : ""}.csv`; a.click();
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("w-[150px] justify-start text-left font-normal", !startDate && "text-muted-foreground")}>
                <CalendarIcon className="h-4 w-4 mr-1" />{startDate ? format(startDate, "dd/MM/yyyy") : "Dari tanggal"}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={startDate} onSelect={setStartDate} initialFocus className={cn("p-3 pointer-events-auto")} /></PopoverContent>
          </Popover>
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("w-[150px] justify-start text-left font-normal", !endDate && "text-muted-foreground")}>
                <CalendarIcon className="h-4 w-4 mr-1" />{endDate ? format(endDate, "dd/MM/yyyy") : "Sampai tanggal"}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={endDate} onSelect={setEndDate} disabled={(date) => startDate ? date < startDate : false} initialFocus className={cn("p-3 pointer-events-auto")} /></PopoverContent>
          </Popover>
          {(startDate || endDate) && <Button variant="ghost" size="sm" onClick={() => { setStartDate(undefined); setEndDate(undefined); }}>Reset</Button>}
          <Button onClick={exportCSV} disabled={stats.length === 0} variant="outline"><Download className="h-4 w-4 mr-1" /> Export CSV</Button>
        </div>
      </div>

      {stats.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-card py-12 shadow-sm">
          <FileBarChart className="h-10 w-10 text-muted-foreground" /><p className="text-muted-foreground">Belum ada data.</p>
        </div>
      ) : (
        <>
                ...
                {stats.map(({ student, hadir, izin, sakit, alpha, percentage }) => (
                  <tr key={student.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${percentage >= 80 ? "bg-success/10 text-success" : percentage >= 50 ? "bg-warning/10 text-warning" : "bg-destructive/10 text-destructive"}`}>{percentage}%</span>
                    </td>
                  </tr>
                ))}
              </tbody>
                    ...
                    <tbody className="divide-y divide-border">
                      {monthlyData.map((m) => (
                        <tr key={m.label} className="hover:bg-muted/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${m.percentage >= 80 ? "bg-success/10 text-success" : m.percentage >= 50 ? "bg-warning/10 text-warning" : "bg-destructive/10 text-destructive"}`}>{m.percentage}%</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
