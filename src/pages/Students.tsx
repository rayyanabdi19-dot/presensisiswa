...
import { getStudents, addStudent, deleteStudent, updateStudent, type Student } from "@/lib/supabase-store";
...
export default function Students() {
  ...
  const filteredStudents = students.filter(
    (s) => s.name.toLowerCase().includes(search.toLowerCase()) ||
      s.class.toLowerCase().includes(search.toLowerCase()) || s.nisn.includes(search)
  );
  ...
  const handleAdd = async () => {
    ...
    const result = await addStudent({ name: form.name, class: form.class, nisn: form.nisn, parentPhone: form.parentPhone || undefined });
    if (result) {
      await loadStudents();
      resetForm();
      setShowAdd(false);
  ...
  const handleEdit = async () => {
    ...
    await updateStudent(editStudent.id, { name: form.name, class: form.class, nisn: form.nisn, parentPhone: form.parentPhone || undefined });
    await loadStudents();
    resetForm();
    setEditStudent(null);
  ...
  const handleDownloadQR = () => {
    ...
    img.onload = () => {
      const padding = 40; const textHeight = 60;
  ...
  const sendWhatsApp = (student: Student, status: string) => {
    if (!student.parentPhone) { toast.error("Nomor WhatsApp orang tua belum diisi."); return; }
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
      ...
      <div className="relative">
        ...
        <Input placeholder="Cari nama, kelas, atau NISN..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" />
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {filteredStudents.length === 0 ? (
                <TableRow><TableCell colSpan={6} className="h-24 text-center text-muted-foreground">{search ? "Tidak ada siswa yang cocok." : "Belum ada data siswa."}</TableCell></TableRow>
              ) : (
                filteredStudents.map((student, idx) => (
                  <TableRow key={student.id} className="hover:bg-muted/10 transition-colors">
                    <TableCell className="text-center text-muted-foreground text-xs">{idx + 1}</TableCell>
                    <TableCell className="font-medium text-foreground">{student.name}</TableCell>
                    <TableCell><span className="inline-flex items-center rounded-md bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">{student.class}</span></TableCell>
                    <TableCell className="font-mono text-sm text-muted-foreground">{student.nisn}</TableCell>
                    <TableCell className="hidden sm:table-cell text-sm text-muted-foreground">{student.parentPhone || <span className="text-muted-foreground/50">—</span>}</TableCell>
                    <TableCell>
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => setQrStudent(student)} className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors" title="QR Code"><QrCode className="h-4 w-4" /></button>
                        <button onClick={() => openEdit(student)} className="rounded-lg p-1.5 text-muted-foreground hover:bg-primary/10 hover:text-primary transition-colors" title="Edit"><Pencil className="h-4 w-4" /></button>
                        {student.parentPhone && (
                          <button onClick={() => sendWhatsApp(student, "Hadir")} className="rounded-lg p-1.5 text-muted-foreground hover:bg-success/10 hover:text-success transition-colors" title="Kirim WhatsApp"><MessageCircle className="h-4 w-4" /></button>
                        )}
                        <button onClick={() => handleDelete(student.id, student.name)} className="rounded-lg p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-colors" title="Hapus"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))
          ...
          {qrStudent && (
            <div className="flex flex-col items-center gap-4 py-4">
              <div ref={qrRef} className="rounded-xl border border-border bg-background p-4">
                <QRCodeSVG value={JSON.stringify({ id: qrStudent.id, name: qrStudent.name, nisn: qrStudent.nisn })} size={200} level="M" />
              </div>
              ...
              <Button onClick={handleDownloadQR} className="w-full"><Download className="mr-2 h-4 w-4" />Download QR Code</Button>
            </div>
          )}
        </DialogContent>
      </Dialog>
